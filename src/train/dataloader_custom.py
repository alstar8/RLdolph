...
for i in range(18):
    DEFAULT_SPC_TOKENS['ATARI_'+str(i)] = 16403 + i
...
def fb_collate_fn(batch):
    ...
    for i, sample in enumerate(batch):
        ...
        left_tokens.append(sample['left_text'])
        image_tokens.append(sample['image'])
        right_tokens.append(sample['right_text'])
        
        left_text_tr = pad_sequence(left_tokens, batch_first=True)
        image_tr = torch.stack(image_tokens)
        right_text_tr = pad_sequence(right_tokens, batch_first=True)
        
        
    return left_text_tr, image_tr, right_text_tr